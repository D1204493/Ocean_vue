  ...
  <div class="announcement-section">
    ...
    <div v-if="filteredActivities.length">
      <table class="table table-hover">
        ...
        <tbody>
          <tr v-for="item in filteredActivities" :key="item.id" class="table-row">
...
export default {
  ...
  computed: {
    // 過濾資料，只顯示 ACTIVITY 且 status 為 active 的項目
    filteredReports() {
      if (!this.museumPosts) return [];
    return this.museumPosts.filter(post => 
      post.type === 'ACTIVITY' && post.status === 'active'
      );
    },
  },
  methods: {
