  ...
  <div class="announcement-section">
    ...
    <div v-if="filteredReports.length">
      <table class="table table-hover">
        ...
        <tbody>
          <tr v-for="item in filteredReports" :key="item.id" class="table-row">
...
export default {
  ...
  computed: {
    // 過濾資料，只顯示 REPORT 且 status 為 active 的項目
    filteredReports() {
      if (!this.museumPosts) return [];
    return this.museumPosts.filter(post => 
      post.type === 'REPORT' && post.status === 'active'
      );
    },
  },
  methods: {
